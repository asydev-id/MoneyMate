# 📌 Daftar Situs Pengembangan MoneyMate

Berikut adalah kumpulan link pengembangan yang dikelompokkan berdasarkan kategori.

## 🛠️ Pengembangan (Development)
* [weatherly-nontarnished-lorinda.ngrok-free.dev](https://weatherly-nontarnished-lorinda.ngrok-free.dev) – Lingkungan Testing Local Development.
* [moneymate-dev.up.railway.app](https://moneymate-dev.up.railway.app) – Demo Early Access Improvement Update.
* [polibatam.id/moneymatedev](https://polibatam.id/moneymatedev) – Shortlink untuk kebutuhan Local Testing.

## 🧪 Live Testing / Demo
* [moneymate.sroft.my.id](https://moneymate.sroft.my.id) – Lingkungan Beta Test mendekati produksi.
* [polibatam.id/moneymate](https://polibatam.id/moneymate) – Shortlink untuk kebutuhan Beta Test ataupun Live Demo.
* [moneymate-test.up.railway.app](https://moneymate-test.up.railway.app) – Testing dengan lingkungan cloud.

## 📚 Redirect / Backup
* [moneymate-id.up.railway.app](https://moneymate-id.up.railway.app) – Redirect terbatas jika server utama maintenance.

## 🚀 Production / Publish
* [moneymate.id](https://moneymate.id) – Deployment Server Utama Production.
* [polibatam.id/moneymate-admin](https://polibatam.id/moneymate-admin) – Shortlink untuk ke web admin panel moneymate.

## 📢 Promotions

## 🌐 Contacts / Social Media
* Gmail - moneymate.app.id@gmail.com
* [Instagram](https://www.instagram.com/moneymate_id)
* [Twitter](https://x.com/moneymateid)
* [Discord](https://discord.gg/Z9YxgFEVtG)
